...
class GraphicsEngine
{
    ...
    bool Initialize(wl_display *display, wl_surface *surface, int w, int h)
    {
        ...
        if (!InitSharedDisplay(display))
            return false;
        if (!ChooseSharedConfig())
            return false;
    ...
    void EndFrame() { eglSwapBuffers(shared_egl_display, egl_surface); }
    ...
    bool ChooseSharedConfig()
    {
        ...
        if (!eglChooseConfig(shared_egl_display, attribs, &shared_egl_config, 1, &num_configs)
            || num_configs == 0) {
    ...
    bool CreateSharedContext()
    {
        ...
        shared_egl_context = eglCreateContext(shared_egl_display, shared_egl_config,
    ...
    bool CreateSurface(wl_surface *surface)
    {
        ...
        egl_surface
            = eglCreateWindowSurface(shared_egl_display, shared_egl_config,
