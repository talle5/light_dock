...
class WindowTracker
{
    ...
    static void manager_handle_toplevel(void *data, zwlr_foreign_toplevel_manager_v1 *manager,
                                        zwlr_foreign_toplevel_handle_v1 *toplevel)
    { static_cast<WindowTracker *>(data)->OnToplevelCreated(toplevel); }

    static void manager_handle_finished(void *data, zwlr_foreign_toplevel_manager_v1 *manager)
    { static_cast<WindowTracker *>(data)->OnManagerFinished(); }

    static void handle_title_c(void *data, zwlr_foreign_toplevel_handle_v1 *toplevel,
                               const char *title)
    { static_cast<WindowTracker *>(data)->OnTitleChanged(toplevel, title); }

    static void handle_app_id_c(void *data, zwlr_foreign_toplevel_handle_v1 *toplevel,
                                const char *app_id)
    { static_cast<WindowTracker *>(data)->OnAppIdChanged(toplevel, app_id); }

    static void handle_output_enter_c(void *data, zwlr_foreign_toplevel_handle_v1 *toplevel,
                                      wl_output *output)
    {
    }

    static void handle_output_leave_c(void *data, zwlr_foreign_toplevel_handle_v1 *toplevel,
                                      wl_output *output)
    {
    }

    static void handle_state_c(void *data, zwlr_foreign_toplevel_handle_v1 *toplevel,
                               wl_array *state)
    { static_cast<WindowTracker *>(data)->OnStateChanged(toplevel, state); }

    static void handle_done_c(void *data, zwlr_foreign_toplevel_handle_v1 *toplevel) {}

    static void handle_closed_c(void *data, zwlr_foreign_toplevel_handle_v1 *toplevel)
    { static_cast<WindowTracker *>(data)->OnClosed(toplevel); }

    static void handle_parent_c(void *data, zwlr_foreign_toplevel_handle_v1 *toplevel,
    ...
    void Initialize(wl_registry *registry) { (void)registry; }
    ...
    void OnTitleChanged(zwlr_foreign_toplevel_handle_v1 *toplevel, const char *title)
    {
        ...
        std::cout << "[WindowTracker] title updated: handle=" << toplevel << " title=\""
                  << app.title << "\"\n";
    }
    ...
    void OnAppIdChanged(zwlr_foreign_toplevel_handle_v1 *toplevel, const char *app_id)
    {
        ...
        std::cout << "[WindowTracker] app_id updated: handle=" << toplevel << " app_id=\""
                  << app.app_id << "\"\n";
    }
    ...
    [[nodiscard]] bool ToggleMinimizeOrActivateApp(const std::string_view app_id,
                                                   wl_seat *seat) const
    {
        if (!seat || app_id.empty()) {
            return false;
    ...
    void HandleRegistryGlobal(wl_registry *registry, uint32_t name, const char *interface,
                              uint32_t version)
    {
        ...
        m_manager = static_cast<zwlr_foreign_toplevel_manager_v1 *>(wl_registry_bind(
            registry, name, &zwlr_foreign_toplevel_manager_v1_interface, bind_version));
        std::cout << "[WindowTracker] foreign toplevel manager bound\n";

        static constexpr zwlr_foreign_toplevel_manager_v1_listener manager_listener
            = {.toplevel = manager_handle_toplevel, .finished = manager_handle_finished};
    ...
    void OnClosed(zwlr_foreign_toplevel_handle_v1 *toplevel)
    {
        ...
        std::cout << "[WindowTracker] window closed: handle=" << toplevel << " title=\""
                  << it->second.title << "\" app_id=\"" << it->second.app_id << "\"\n";
