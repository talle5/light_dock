...
{
    public:
    enum class SurfaceId { TopPanel, Launcher, Count };

    struct LayerRole {
        zwlr_layer_surface_v1 *handle = nullptr;
    };

    struct PopupRole {
        xdg_surface *xdg_surface_handle = nullptr;
        xdg_popup *xdg_popup_handle     = nullptr;
    };

    struct SurfaceState {
        wl_surface *surface = nullptr;
        std::variant<std::monostate, LayerRole, PopupRole> role;
    ...
    struct Callbacks {
        void (*move)(double x, double y, void *data)              = nullptr;
        void (*click)(double x, double y, int button, void *data) = nullptr;
        void (*leave)(void *data)                                 = nullptr;
    };
    ...
    static void pointer_handle_enter(void *data, wl_pointer *, uint32_t, wl_surface *,
                                     wl_fixed_t sx, wl_fixed_t sy)
    {
        ...
        if (ctx->m_callbacks.move)
            ctx->m_callbacks.move(ctx->m_mouse_x, ctx->m_mouse_y, ctx->m_callback_data);
    }
    ...
    static void pointer_handle_button(void *data, wl_pointer *, uint32_t, uint32_t,
                                      uint32_t button, uint32_t state)
    {
        if (const auto *ctx = static_cast<WaylandContext *>(data);
            state == WL_POINTER_BUTTON_STATE_PRESSED && ctx->m_callbacks.click) {
            const int b = (button == BTN_LEFT) ? 1 : (button == BTN_RIGHT ? 3 : 0);
            ctx->m_callbacks.click(ctx->m_mouse_x, ctx->m_mouse_y, b, ctx->m_callback_data);
        }
    }

    static void pointer_handle_leave(void *data, wl_pointer *, uint32_t, wl_surface *)
    {
        if (const auto *ctx = static_cast<WaylandContext *>(data); ctx->m_callbacks.leave)
            ctx->m_callbacks.leave(ctx->m_callback_data);
    }
                                       ...
                                       const char *interface, uint32_t version)
    {
        ...
        if (strcmp(interface, wl_compositor_interface.name) == 0)
            ...
            ctx->m_layer_shell = static_cast<zwlr_layer_shell_v1 *>(
    ...
    void SetCallbacks(const Callbacks &cb, void *data)
    {
        m_callbacks     = cb;
    ...
    wl_surface *CreateSurface(SurfaceId id, uint32_t w, uint32_t h, uint32_t anchor)
    {
        ...
        LayerRole role;
        role.handle = zwlr_layer_shell_v1_get_layer_surface(
    ...
    [[nodiscard]] const SurfaceState &GetSurfaceState(SurfaceId id) const
    { return m_surfaces[static_cast<size_t>(id)]; }
    ...
    ~WaylandContext()
    {
        for (const auto &s : m_surfaces) {
            if (auto *role = std::get_if<LayerRole>(&s.role)) {
                if (role->handle)
                    zwlr_layer_surface_v1_destroy(role->handle);
            } else if (auto *p = std::get_if<PopupRole>(&s.role)) {
                if (p->xdg_popup_handle)
                    xdg_popup_destroy(p->xdg_popup_handle);
                if (p->xdg_surface_handle)
                    xdg_surface_destroy(p->xdg_surface_handle);
            }
            if (s.surface)
