...
namespace unity::shell
{
    ...
    public:
        ...
        void SetGeometry(const nux::Rect& geometry)
        {
            m_geometry = geometry;
        }

        void AddIcon(const LauncherIcon& icon)
        {
        ...
        {
            return std::ranges::any_of(m_icons, [&](const auto& icon)
            {
                return icon.GetAppId() == app_id;
            });
        }
        ...
        void ComputeLayoutAndDraw(Renderer2D& renderer)
        {
            ...
            for (size_t i = 0; i < m_icons.size(); ++i)
            {
                ...
                const float current_y = layout.icons_y + (layout.dock_height - (m_layout.vertical_padding * 2.0f) -
                    icon_size) * 0.5f;
    ...
    private:
        [[nodiscard]] ComputedLayout CalculateLayout() const
        {
            ...
            const float base_total_width =
                (count * base_size) + ((count - 1.0f) * spacing);
            ...
            layout.dock_y =
                static_cast<float>(m_geometry.y + m_geometry.h) - layout.dock_height - kShadowPadding;
